...
const mapper = async (employee: TBambooHREmployee): Promise<TBambooHREmployeeExtended> => {
...
export const getEmployees = async (countryList: string[]): Promise<TBambooHREmployeeExtended[]> => {
  ...
  employees = await pMap((data as TBambooHREmployeesDirectory).employees, mapper, { concurrency: CONCURRENCY });
...
export const getEmployeesAtOffice = async (today: moment.Moment): Promise<TBambooHREmployeeAtOffice[]> => {
...
export const getBankHolidays = async (today: moment.Moment): Promise<TWhosOut> => {
  ...
  return {
    next: (whosOutData as TBambooHRWhosOut[]).filter(e => e.type === BAMBOOHR_TIME_WHOS_OUT_TYPE),
    month: (whosOutThisMonthData as TBambooHRWhosOut[]).filter(e => e.type === BAMBOOHR_TIME_WHOS_OUT_TYPE)
  };
};
