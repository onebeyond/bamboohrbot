...
import { TBambooHREmployeeAtOffice, TBambooHREmployeeExtended, TWhosOut } from ".";
import { FRIDAY_ISO_WEEKDAY, HUMAN_READABLE_DATE, MONDAY_ISO_WEEKDAY, ordinalSuffixOf, YEAR_MONTH_DATE_FORMAT } from "./common";
...
export const pusblishBankHolidays = async (
  ...
  if (bankHolidays.month.length > 0 && 
    ...
    (today.date() === 3 && today.isoWeekday() === MONDAY_ISO_WEEKDAY))) {
      ...
      monthBankHolidaysBlocks.push(...bankHolidays.month.map(holiday => ({
        "type": "section",
        "text": {
          "type": "mrkdwn",
          "text": `*${holiday.name}*\n${moment(holiday.start).format(HUMAN_READABLE_DATE)}`
        }
      })));
...
export const publishEmployeesAtOffice = async (
  ...
  const filteredEmployeesAtOffice = employeesAtOffice.reduce<TBambooHREmployeeExtended[]>((previousValue, currentEmployee) => {
  ...
  const blocks = filteredEmployeesAtOffice.length > 0
  ...
  const message = {
    "text": `🏢 ${today.isoWeekday() < FRIDAY_ISO_WEEKDAY
      ? 'Tomorrow'
      : 'Next Monday'} at One Beyond offices`,
    "blocks": [
      {
        "type": "header",
        "text": {
  ...
  await postSlackMessage(
    process.env.OFFICE_WEBHOOK_URL ?? '',
    message
  );
}
...
export const publishEmployeesCelebrations = async (
  ...
  if (firstDayEmployees.length > 0) {
    ...
    firstDayBlocks.push(...firstDayEmployees.map(employee => ({
      "type": "context",
      "elements": [
        {
          "type": "image",
  ...
  const birthdays = employees
    ...
    .reduce<TBambooHREmployeeExtended[]>((previousValue, employee) => {
      const birthdayWithCurrentYear: string = `${today.year()}-${employee.birthday}`;

      if (!moment(birthdayWithCurrentYear).isValid()) {
        return previousValue;
      }

      if (today.isoWeekday() <= FRIDAY_ISO_WEEKDAY && moment(birthdayWithCurrentYear).isSame(today, 'day')) {
        return previousValue.concat(employee);
      }

      if (today.isoWeekday() === FRIDAY_ISO_WEEKDAY && moment(birthdayWithCurrentYear).isBetween(moment(today).add(1, 'days'), moment(today).add(2, 'days'), 'day', '[]')) {
  ...
  if (birthdays.length > 0) {
    ...
    birthdaysBlocks.push(...birthdays.map(employee => ({
      "type": "context",
      "elements": [
        {
          "type": "image",
        ...
        {
          "type": "mrkdwn",
          "text": `*Happy birthday ${employee.birthdayOn ? `on ${employee.birthdayOn}` : ''}, ${employee.displayName}!*`
        }
      ]
    })));
  ...
  const anniversaries = employees
    ...
    .reduce<TBambooHREmployeeExtended[]>((previousValue, employee) => {
      ...
      const hireDateMonth = (moment(employee.hireDate).month() + 1).toString().padStart(2, '0');
      const hireDateDay = moment(employee.hireDate).date().toString().padStart(2, '0');
      ...
      if (today.isoWeekday() <= FRIDAY_ISO_WEEKDAY && moment(hireDateWithCurrentYear).isSame(today, 'day')) {
          return previousValue.concat({
            ...employee,
            anniversary: ordinalSuffixOf(today.year() - moment(employee.hireDate).year()),
          });
      }

      if (today.isoWeekday() === FRIDAY_ISO_WEEKDAY && moment(hireDateWithCurrentYear).isBetween(moment(today).add(1, 'days'), moment(today).add(2, 'days'), 'day', '[]')) {
          return previousValue.concat({
            ...employee,
            anniversary: ordinalSuffixOf(today.year() - moment(employee.hireDate).year()),
  ...
  if (anniversaries.length > 0) {
    ...
    anniversariesBlocks.push(...anniversaries.map(employee => ({
      "type": "context",
      "elements": [
        {
          "type": "image",
        ...
        {
          "type": "mrkdwn",
          "text": `*Happy ${employee.anniversary} anniversary ${employee.anniversaryOn ? `on ${employee.anniversaryOn}`: ''}, ${employee.displayName}!*`
        }
      ]
    })));
